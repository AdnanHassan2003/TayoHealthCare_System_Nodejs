...
//                     if (doctorData.availability[day].slots) {
//                         let startTimes = doctorData.availability[day].slots.start_time;
//                         let endTimes = doctorData.availability[day].slots.end_time;
                        
//                         // If startTimes and endTimes are not arrays, wrap them in arrays
//                         if (!Array.isArray(startTimes)) startTimes = [startTimes];
//                         if (!Array.isArray(endTimes)) endTimes = [endTimes];
        
//                         // Convert into structured slot objects
//                         doctorData.availability[day].slots = startTimes.map((startTime, index) => ({
...
//             console.log(doctorData); // Check if the structure is now correct
...
// exports.save_doctor_data = async function (req, res) {
//     try {
//         Utils.check_admin_token(req.session.admin, async function (response) {
//             console.log("body", req.body);

//             if (!response.success) {
//                 return Utils.redirect_login(req, res);
//             }

//             let doctorData = req.body;

//             // **Hagaajinta `availability` si ay sax ugu qaabeysnaato**
//             let formattedAvailability = {};
//             if (doctorData.availability) {
//                 Object.keys(doctorData.availability).forEach(day => {
//                     let dayData = doctorData.availability[day];
//                     if (dayData && dayData.slots) {
//                         let startTimes = dayData.slots.start_time || [];
//                         let endTimes = dayData.slots.end_time || [];

//                         // Hubi in `start_time` iyo `end_time` ay yihiin arrays
//                         if (!Array.isArray(startTimes)) startTimes = [startTimes];
//                         if (!Array.isArray(endTimes)) endTimes = [endTimes];

//                         // Isku qor slots sax ah
//                         let slots = startTimes.map((startTime, index) => ({
//                             start_time: startTime,
//                             end_time: endTimes[index] || ""
//                         }));
...
//                 });
//             }
//             doctorData.availability = formattedAvailability;

//             // **Hubi haddii doctor horay u jiro**
//             let existingDoctor = await Doctor.findOne({ phone: req.body.phone });
//             if (existingDoctor) {
//                 req.session.error = "Sorry, There is a doctor with this phone, please check the phone";
//                 return Utils.redirect_login(req, res);
//             }

//             // **Hubi in password uu sax yahay**
//             var schema = new passwordValidator();
//             schema.is().min(8)               // Ugu yaraan 8 xaraf
//                   .is().max(30)              // Ugu badnaan 30 xaraf
//                   .has().lowercase()         // Waa inuu leeyahay xarfo yaryar
//                   .has().digits()            // Waa inuu leeyahay tirooyin
//                   .has().not().spaces();     // Waa inuusan lahayn meel banaan

//             if (!schema.validate(req.body.password)) {
//                 req.session.error = "Please use a strong password that contains letters and digits";
//                 return res.redirect("/add_doctor");
//             }

//             // **Hadii password uu sax yahay, keydi doctor**
...
//             // **Haddii doctor uu leeyahay profile picture, kaydi sawirka**
//             if (req.files && req.files.length > 0) {
//                 let profileFile = req.files[0];
//                 let imageName = Utils.tokenGenerator(29) + ".jpg";
//                 let imagePath = "./uploads/admin_profile/" + imageName;

//                 fs.readFile(profileFile.path, function (err, data) {
//                     if (!err) {
//                         fs.writeFile(imagePath, data, 'binary', function (err) {
//                             if (!err) {
//                                 doctor.picture = "admin_profile/" + imageName;
//                             }
//                         });
//                         fs.unlink(profileFile.path, function () {});
//                     }
//                 });
//             }
...
//             // **Badbaadi doctor**
//             await doctor.save();
//             req.session.success = "Congrats, Doctor was created successfully!";
//             res.redirect("/doctor_list");
//         });
...
//         console.log("Availability Data:", req.body.availability);
...
//                 Object.keys(doctorData.availability).forEach(day => {
//                     if (doctorData.availability[day].slots) {
//                         let slots = doctorData.availability[day].slots;
            
//                         if (!Array.isArray(slots)) {
//                             slots = [slots]; // Ensure slots is always an array
//                         }
...
//                 doctorData.availability = formattedAvailability; // Set the properly formatted availability
//             }
